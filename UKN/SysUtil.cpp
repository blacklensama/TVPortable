...
namespace ukn {
    ...
    mFeatureMask(0) {
        ...
        *reinterpret_cast<uint32_t*>(&mCPUString[8]) = cpuid.Ecx();
        ...
        mNumHWThreads = (int)sysconf(_SC_NPROCESSORS_CONF);	// This will tell us how many CPUs are currently enabled.
#endif
...
#elif defined(UKN_OS_LINUX) || defined(UKN_OS_OSX) || defined(UKN_OS_IOS)
            {
                bool supported = (GenuineIntel == mCPUString) || (AuthenticAMD == mCPUString);
